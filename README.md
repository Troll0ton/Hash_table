...
Первая часть проекта посвящена изучению работ различных хэш-функций. Необходимо было исследовать эффективность работы хэш-функций на тексте - проанализировать распространенность коллизий в каждом отдельном списке хэш-таблицы. 
...
Я использовал **Callgrind** - инструмент для анализа производительности программного обеспечения. Он использует эмуляцию процессора, доступную в **Valgrind**, что позволяет перехватить все обращения к памяти без перекомпиляции программы. **Callgrind** производит профилирование основываясь на брейкпоинтах на инструкциях типа call и ret. Трассировка определяет число инструкций/обращений к данным, уровень непопаданий в кэш (1-го или 2-го уровня) и устанавливает соответствующее место в исходных кодах. К минусам можно отнести медлительность эмуляции процессора (снижение скорости в 50 раз). 

Измерение уровня производительности вычисляется в **стоимости** определенных процессов. Такая величина включает в себя не только измерение числа выполненных инструкций, но и время обращения к памяти и другие процессы.
...
### Тестирующая система <a name="testing"></a>

Все хэш-функции проверялись на трагедии Уильяма Шекспира "Трагическая история о Гамлете, принце датском", представленном на анлийском языке. Для этого с помощью ранее написанных утилит, позволяющих разделить текст на отдельные слова, производилось внесение этих элементов в хэш-таблицу.

<details>
	<summary>bufferSeparator () <a name="bufferSeparator"></a></summary>  
...
<details>
	<summary>fillHashTable () <a name="fillHashTable"></a></summary>  
...
{
    int total = 0;
	// buffer - массив из указателей на слова
...
Так как последующая оптимизация программы затрагивает ускорение работы функции поиска по хэш-таблице, поиск осуществлялся несколько тысяч раз. Это в дальнейшем позволило оценить вклад, который вносила соответствующая оптимизация в работу программы, и исключить влияние остальных функций.
<details>
	<summary>searchingAll () <a name="searchingAll"></a></summary>  
...
Компиляция производилась со следующими флагами:
...
В дальнейшем будут представлены различные оптимизации, влияющие на скорость работы программы как в лучшую сторону, так и худшую стороны, сделаны соответствующие выводы об уместности из применения в определенных ситуациях. 
...
| режимы        | стоимость          | ускорение          |
| ------------- | ------------------ | ------------------ |
| NO AVX        | 5765331050         | 1.0                |
| AVX           | 4215002668         | 1.36               |

После проведения соответсвующих оптимизаций скорость работы программы увеличилась на 36 процентов. 

Оказалось, что для удобной и красивой обработки входных данных достаточно преобразовать исходные данные так, что в дальнейшем с ними было удобно оперировать.

Хотя и время, которое тратится на преобразование входных данных увеличилось, время работы программы уменьшилось. Так работает закон **Амдала**. Таким образом, нам опять пригодилось использование **AVX**-инструкций, которые хорошо подходят для обработки большого числа данных.
...
Было решено переписать её в ассемблерном виде и вызывать её уже в самой программе. 
...
Как оказалось, после применения соответствующей оптимизации, производительность программы упала. Наша хэш-функция оказалась недостаточно оптимизированной, чтобы обогнать **-O2**.
...
![type4](images/type4.crc.png)

Использование такой хэш-функции на преобразованных в 256-битный формат данных позволило повысить производительность программы приблизительно в 1.5 раза.
...
В итоге производительность программы была увеличена в 2.1 раза. Здесь сильно помогло преобразование исходных данных, использование интрисиков. Применение оптимизаций не всегда даёт положительный результат, в частности, при использовании ассемблерных функций и вставок. 

---

## Заключение <a name="conc"></a>
В данной работе были исследованы устройство хэш-таблиц, работы хэш-функций, было проведено их сравнение. Также активно проводился анализ работы программы через профайлер, изучено влияние отдельных компонентов программы. В заключение хочется отметить, что выгодно преобразовать данные так, чтобы их можно было удобнее обрабатывать. На текущей задаче были использованы SIMD-инструкции, которые позволили ускорить программу в 2.1 раз. 
